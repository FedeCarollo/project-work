...
This approach leverages **Swarm Intelligence**, inspired by the foraging behavior of real ants finding the shortest path
between their nest and a food source via pheromone trails. In our context, ACO is used to solve the problem by combining
topological optimization (visiting order) with logistic weight management.

### Algorithm Overview

The algorithm operates on a **Constructive & Refine** paradigm: instead of improving a single complete solution (like in
GA), it probabilistically builds multiple solutions at every iteration and reinforces the components of the best ones.

1. **Solution Construction (The Giant Tour)**
    * A colony of $m$ agents ("ants") is placed randomly on the graph.
    * Each ant builds a complete permutation of cities (Giant Tour) by selecting the next city $j$ from the current city
      $i$ using a probabilistic transition rule:
      $$P_{ij} = \frac{(\tau_{ij})^\alpha \cdot (\eta_{ij})^\beta}{\sum (\tau_{ik})^\alpha \cdot (\eta_{ik})^\beta}$$
    * Where $\tau_{ij}$ represents the **pheromone level** (historical memory of the colony) and $\eta_{ij}$ represents
      the **heuristic information** (local visibility, typically $1/d_{ij}$).

2. **Evaluation via Split Algorithm**
    * Once an ant generates a sequence (e.g., $A \to B \to C \to D$), this is not yet a valid path as it ignores depot
      returns for unloading.
    * We employ **Prins' Split Algorithm** (see ILS section) to optimally segment this sequence into feasible trips,
      calculating the actual cost based on the weight function $w^\beta$.

3. **Pheromone Update**
    * **Evaporation:** All pheromones on the graph are reduced by a factor $\rho$ to "forget" bad past choices and
      encourage exploration.
    * **Deposit:** Only the ants that found the best paths deposit new pheromones on the traversed edges. The amount is
      proportional to the solution quality ($Q / Cost$).

### Key Features

#### 1. Hybrid Split Strategy (Beta Management)

The problem presents a unique challenge: when $\beta \ge 2$, the cost grows quadratically with weight. Standard ACO
would fail to evaluate edges correctly. We implemented two corrective mechanisms:

* **Split with Hard Pruning:** During tour segmentation, if the marginal cost of continuing with the current load
  exceeds a heuristic threshold (based on empty travel), the algorithm forces a return to the depot. This prevents the
  exploration of "explosive" high-cost trips and speeds up computation.
* **Beta Optimizer Integration:** The best solution found by the ants is passed to the *Linear Scan Optimizer* (
  analytical) to surgically refine the trip cut-off points.

#### 2. Feasibility on Sparse Graphs

Since the graph is not complete (density < 1.0), an ant might decide to visit city $B$ after city $A$ even if no direct
edge exists.

* The solver includes a **Physical Reconstruction** module: it converts the logical sequence (target cities) into a real
  physical path using pre-computed shortest paths (Dijkstra), ensuring every solution is 100% feasible.

#### 3. Adaptive Exploration/Exploitation

* For simple instances ($\beta \le 1$), the algorithm favors heuristic information (geometric distance).
* For complex instances ($\beta \ge 2$), the algorithm increases the pheromone weight ($\alpha_{ACO}$), relying more on
  the "collective memory" of previous iterations rather than local greediness, which would lead to expensive dead ends.

### Complexity

* **Construction:** $O(m \cdot N^2)$ per iteration, where $m$ is the number of ants.
* **Split Algorithm:** $O(N \cdot B)$, where $B$ is the lookahead buffer (effective constant time).
* **Overall:** ACO is computationally heavier than the greedy Merge approach but offers superior capability in finding
  global optima in complex topologies where the visitation order is critical.
